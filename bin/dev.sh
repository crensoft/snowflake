...
## CONF ##
function init() {
  ensureBuild "$SVC_NAME-id-dev"
  ensureBuild "$SVC_NAME-test"
}
# /CONF ##

function buildImage() {
  if [[ $1 =~ -dev$ && -f ${1/-dev/""}/Dockerfile.dev ]]; then
    docker build -t $APP_SLUG/$1 -f ${1/-dev/""}/Dockerfile.dev ${1/-dev/""}
  else
...
if [[ ( $1 == "start" ) ]]; then
  docker-compose -f docker-compose.dev.yaml run --rm test yarn test:watch
elif [[ ( $1 == "stop" )]]; then
